...
public class OrdenesServices {
    ...
    public Ordenes finalizarCompra(Integer idUsuario, Integer idDireccion){
        ...
        if(carrito.getDetalles().isEmpty()) throw new RuntimeException("El carrito está vacío");
        ...
        for(CarritoDetalle item : carrito.getDetalles()) {
            ...
            if(variante.getStock() < item.getCantidad()) 
                throw new RuntimeException("No hay suficiente stock para el producto: " + variante.getProducto().getNombre());
